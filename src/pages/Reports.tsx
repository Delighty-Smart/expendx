  ...
  const exportToPDF = async () => {
    try {
      ...
      (allTransactions || []).forEach((t, index) => {
        ...
        const amountStr = `${currency.symbol}${t.amount.toLocaleString()}`;

        pdf.text(dateStr, margin + 2, yPosition);
        pdf.text(t.category.substring(0, 15), margin + 30, yPosition);
        pdf.text((t.description || '-').substring(0, 30), margin + 70, yPosition);

        pdf.setTextColor(t.type === 'credit' ? 16 : 239, t.type === 'credit' ? 185 : 68, t.type === 'credit' ? 129 : 68);
        pdf.text(t.type.toUpperCase(), margin + 140, yPosition);
