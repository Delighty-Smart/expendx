...
const ReportsPage = () => {
  ...
  const exportData = async () => {
    try {
      ...
      a.href = url;
      a.download = `Lucent_Export_${format(dateFrom, 'yyyy-MM-dd')}_to_${format(dateTo, 'yyyy-MM-dd')}.csv`;
      a.click();
  ...
  const exportToPDF = async () => {
    try {
      ...
      const drawFooter = (pageNum: number) => {
        ...
        pdf.text(`Generated by Lucent | Page ${pageNum}`, pageWidth / 2, pageHeight - 10, { align: "center" });
      };
      ...
      try {
        toast({
          ...
          description: "Lucent Intelligence is generating your insights...",
        });
      ...
      drawFooter(pdf.internal.pages.length - 1);
      pdf.save(`Lucent_Report_${format(dateFrom, 'yyyy-MM-dd')}_to_${format(dateTo, 'yyyy-MM-dd')}.pdf`);
