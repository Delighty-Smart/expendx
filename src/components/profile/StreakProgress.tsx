      ...
      <GlassCard className="overflow-hidden">
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-7 gap-3">
            ...
            const isToday = index === todayIndex;
            return <div key={index} className="flex flex-col items-center gap-2">
                  <div className={cn("w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300", isChecked ? "bg-gradient-to-r from-green-400 to-emerald-500 shadow-md shadow-green-500/25" : "bg-white/10 backdrop-blur-sm border border-white/20", isToday && "ring-2 ring-pink-500 ring-offset-1 ring-offset-transparent scale-110")}>
                    {isChecked ? <Check className="h-4 w-4 text-white" /> : isToday ? <div className="w-2 h-2 rounded-full bg-pink-500" /> : null}
                  </div>
                  <span className={cn("text-xs font-medium", isToday ? "text-pink-500" : "text-muted-foreground")}>
                    {day}
                  </span>
                </div>;
          })}
