...
export function PageHeader({ title, subtitle, actions, className, backTo, backHistory, hideBack }: PageHeaderProps) {
  ...
  const handleBack = () => {
    if (backTo) {
      navigate(backTo);
    } else if (backHistory && window.history.length > 1) {
      navigate(-1);
    } else {
      navigate("/dashboard");
    }
      ...
      <div className="flex items-center justify-between gap-2 min-w-0">
        <div className="flex items-center gap-1.5 min-w-0">
          {showBack && (
            <button
              onClick={handleBack}
              className="flex items-center justify-center w-6.5 h-6.5 sm:w-7 sm:h-7 -ml-0.5 bg-muted/60 hover:bg-muted dark:bg-muted/30 text-foreground border border-border/40 transition-all touch-manipulation shrink-0 active:scale-95"
              style={{ borderRadius: "9999px" }}
              aria-label="Go back"
            >
              <ChevronLeft className="h-3.5 w-3.5 text-foreground" strokeWidth={2.2} />
            </button>
          )}
          <div className="min-w-0">
            <h1 className="text-base sm:text-lg font-bold tracking-tight text-foreground leading-tight truncate">{title}</h1>
            {subtitle ? (
